...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderActivityLogs(appointments) {
    ...
    if (items.length < 3) {
        items.push(
            { icon: 'flask-outline', color: 'text-purple-500', text: 'New prescription added for #P0042', time: '1 hour ago' },
            { icon: 'shield-checkmark-outline', color: 'text-green-500', text: 'Medical record #R881 finalized', time: '2 hours ago' },
            { icon: 'alert-circle-outline', color: 'text-yellow-500', text: 'Low stock alert: Paracetamol 500mg', time: '3 hours ago' }
        );
    }
  ...
  function renderAllPrescriptions(prescriptions) {
    ...
    if (!prescriptions || prescriptions.length === 0) {
      listContainer.innerHTML = '<p class="text-gray-500 py-6 text-center">No prescriptions created yet.</p>';
      return;
    }
    let html = '<table class="appt-table"><thead><tr><th>Date</th><th>Patient</th><th>Medication</th><th>Dosage</th><th>Freq.</th></tr></thead><tbody>';
  ...
  function renderAllAppointments(groups) {
    ...
    categories.forEach((cat) => {
      ...
      appointments.forEach((appt) => {
        ...
        let statusBadge = "";
        if (appt.status === "completed") statusBadge = '<span class="badge badge-green">Completed</span>';
        else if (appt.status === "cancelled" || appt.status === "terminated") statusBadge = '<span class="badge badge-red">Terminated</span>';
        else if (appt.status === "pending") statusBadge = '<span class="badge badge-orange">Pending</span>';
        else if (appt.status === "accepted") statusBadge = '<span class="badge badge-blue">Accepted</span>';
        else statusBadge = `<span class="badge badge-gray">${appt.status.toUpperCase()}</span>`;
  ...
  function fetchPatientHistory(patientId) {
    ...
    list.innerHTML = '<p class="text-center text-gray-400 py-4 italic text-sm animate-pulse">Fetching history...</p>';

    fetch(`../php/doctor_api.php?action=get_patient_history&patient_id=${patientId}`)
      .then(res => res.json())
      .then(result => {
        if (result.status === 'success') {
          const data = result.data;
          if (data.records.length === 0) {
            list.innerHTML = '<p class="text-center text-gray-500 py-6 text-sm">No previous health records found.</p>';
            return;
          }

          let html = '';
          data.records.forEach(r => {
            const date = new Date(r.record_date).toLocaleDateString('en-GB', { day:'2-digit', month:'short', year:'numeric' });
            html += `
      ...
      .catch(err => {
        ...
        list.innerHTML = '<p class="text-center text-red-500 text-xs">Failed to load history.</p>';
      });
  }
  ...
  window.showPatientDetail = function(encodedData) {
    ...
    document.getElementById('pd-name').textContent = appt.patient_name || 'Unknown';
    document.getElementById('pd-avatar').textContent = (appt.patient_name || '?')[0].toUpperCase();
    ...
    document.getElementById('pd-gender').textContent = appt.gender ? appt.gender.charAt(0).toUpperCase() + appt.gender.slice(1) : '—';
    document.getElementById('pd-blood').textContent = appt.blood_type || '—';
    document.getElementById('pd-appt-type').textContent = appt.appointment_type || '—';
  ...
  document.getElementById('add-prescription-toggle')?.addEventListener('change', function(e) {
  ...
  document.getElementById('clinical-action-form')?.addEventListener('submit', function(e) {
    e.preventDefault();
    ...
    formData.append('record_type', document.getElementById('form-record-type').value);
    formData.append('diagnosis', document.getElementById('form-diagnosis').value);
      ...
      .then(data => {
        if (data.status === 'success') {
          ...
          if (document.getElementById('add-prescription-toggle').checked) {
            ...
            rxData.append('medication_name', document.getElementById('form-medication').value);
            rxData.append('dosage', document.getElementById('form-dosage').value);
            rxData.append('frequency', document.getElementById('form-frequency').value);
            rxData.append('duration', document.getElementById('form-duration').value);
            rxData.append('instructions', document.getElementById('form-instructions').value);

            return fetch('../php/doctor_api.php', { method: 'POST', body: rxData }).then(r => r.json());
          }
          return data;
    ...
    patients.forEach((patient) => {
      ...
      if (patient.status === "completed") {
        ...
        actionBtn = `
            ...
            `;
      } else if (patient.status === "accepted" || patient.status === "delayed") {
        actionBtn = `
                <div class="flex items-center gap-2">
  ...
  function initFullCalendar() {
    ...
    if (!calendarEl) {
      ...
      if (apptView) {
        ...
        calContainer.className = 'bg-white dark:bg-gray-800 rounded-xl shadow-lg mt-8 p-6';
        calContainer.innerHTML = '<h3 class="text-xl font-semibold mb-6">📅 Calendar Schedule</h3><div id="calendar" class="min-h-[500px]"></div>';
    ...
    if (targetEl && !isCalendarRendered && typeof FullCalendar !== "undefined") {
      calendarInstance = new FullCalendar.Calendar(targetEl, {
        ...
        events: [
          { title: 'Meeting with Dr. Smith', start: '2025-05-20T10:30:00', backgroundColor: '#3B82F6' },
          { title: 'Surgery - Patient X', start: '2025-05-22T09:00:00', backgroundColor: '#EF4444' },
          { title: 'Conference Call', start: '2025-05-25', backgroundColor: '#10B981' }
        ],
  ...
  window.addEventListener('resize', () => {
    ...
    if (departmentPopularityChartInstance) departmentPopularityChartInstance.resize();
  });
  ...
  document.querySelectorAll('.nav-link[data-view], .sidebar-link[data-view]').forEach(link => {
